...
def clear_transaction_data(db):
    ...
    tables_to_clear = [
        ...
        "supplier_features",
        "supplier_outage_log",
        "supplier_incident_log",
        "supplier_health_snapshot",
        "supplier_prediction_history",
        "wallet_transactions",
        "credit_requests",
        "refund_requests",
        "booking_passengers",
    ...
    try:
        for table in tables_to_clear:
            if table not in existing_tables:
                logger.warning(
                    "Skipping table '%s' because it does not exist.",
                    table,
                )
                continue
        ...
        db.commit()
        logger.info("Old transaction and ML output data cleared successfully.")

    except Exception:
        db.rollback()
        logger.exception("Failed to clear old generated data.")
        raise
...
def temporal_risk_multiplier(supplier_code, event_time):
    """Create repeatable temporary degradation periods across the year.

    This gives the later historical-snapshot trainer changing supplier behavior
    instead of one permanent risk level for the entire dataset.
    """
    day_number = event_time.toordinal()
    supplier_seed = sum(ord(char) for char in supplier_code)
    cycle_position = (day_number + supplier_seed) % 63
...
def choose_booking_status(group, supplier_code, event_time):
    behavior = SUPPLIER_BEHAVIOR[group]
    multiplier = temporal_risk_multiplier(supplier_code, event_time)

    failed = behavior["failed"] * multiplier
    pending = behavior["pending"] * (1.0 + (multiplier - 1.0) * 0.75)
    cancelled = behavior["cancelled"] * (1.0 + (multiplier - 1.0) * 0.45)
    expired = behavior["expired"] * (1.0 + (multiplier - 1.0) * 0.40)
    success = max(0.05, 1.0 - failed - pending - cancelled - expired)

    return random.choices(
        population=["TICKETED", "FAILED", "PENDING", "CANCELLED", "EXPIRED"],
        weights=[success, failed, pending, cancelled, expired],
        k=1,
    )[0]
...
def generate_chunk(
    ...
    for i in range(start_index, end_index):
        ...
        created_at = random_date()
        status = choose_booking_status(group, supplier_code, created_at)

        risk_multiplier = temporal_risk_multiplier(supplier_code, created_at)
        attempts = random.randint(
            behavior["attempt_min"],
            behavior["attempt_max"] + int(risk_multiplier - 1),
        )
...
def main():
    ...
    try:
        ...
        logger.info("Production-style B2B training data generation started...")
        logger.info("Total bookings: %s", TOTAL_BOOKINGS)
        logger.info("Chunk size: %s", CHUNK_SIZE)
        logger.info("Valid users linked to agents: %s", len(users))
        logger.info("Supplier risk mapping:")
        ...
        for start in range(0, TOTAL_BOOKINGS, CHUNK_SIZE):
            ...
            logger.info("Generating records %s to %s...", start + 1, end)
            ...
            logger.info("Chunk completed: %s to %s", start + 1, end)

        logger.info("Production-style B2B data generation completed successfully.")

...
DEFAULTS = {
    ...
    "future_probability_24h": 0,
    "future_probability_3d": 0,
    "future_probability_7d": 0,
...
# Prefer the new current-state anomaly fields, while supporting older API responses.
if "current_anomaly_status" not in df.columns or (
    df["current_anomaly_status"].astype(str).str.strip().eq("").all()
):
    df["current_anomaly_status"] = df["anomaly_status"].map(
        {
            "ANOMALY": "CURRENT_ANOMALY",
            "NORMAL": "CURRENT_NORMAL",
        }
    ).fillna("CURRENT_NORMAL")

if "current_anomaly_score" not in df.columns:
    df["current_anomaly_score"] = df["anomaly_score"]
...
# Prefer the new multi-horizon model outputs. Keep backward compatibility
# with the older single future probability fields.
df["_future_prob_24h"] = df["future_probability_24h"].apply(to_num)
df["_future_prob_3d"] = df["future_probability_3d"].apply(to_num)

df["_future_prob_7d"] = df.apply(
    lambda row: (
        to_num(row.get("future_probability_7d"))
        if to_num(row.get("future_probability_7d")) > 0
        else (
...
# Existing UI sections can continue using _future_prob as the 7-day value.
df["_future_prob"] = df["_future_prob_7d"]
df["future_unavailability_severity"] = (
    df["future_unavailability_severity"]
    .fillna("LOW")
    .astype(str)
    .str.upper()
...
if anomaly_only:
    filtered = filtered[
        filtered["current_anomaly_status"] == "CURRENT_ANOMALY"
    ]
...
<div class="alert-card">
    <div>
        ...
        <div class="alert-row">
            <span><b>24-Hour Risk</b> {fmt_pct(row.get("_future_prob_24h"))}</span>
            <span><b>3-Day Risk</b> {fmt_pct(row.get("_future_prob_3d"))}</span>
            <span><b>7-Day Risk</b> {fmt_pct(row.get("_future_prob_7d"))}</span>
            <span><b>Severity</b> {escape(str(row.get("future_unavailability_severity") or "LOW"))}</span>
            <span><b>Level</b> {risk_badge(row.get("risk_level"))}</span>
            <span><b>Highest-Risk Window</b> {escape(str(row.get("future_risk_window") or "N/A"))}</span>
...
if filtered.empty:
    ...
    table_df["24-Hour Risk"] = table_df["_future_prob_24h"].apply(fmt_pct)
    table_df["3-Day Risk"] = table_df["_future_prob_3d"].apply(fmt_pct)
    table_df["7-Day Risk"] = table_df["_future_prob_7d"].apply(fmt_pct)
    table_df["Severity"] = table_df["future_unavailability_severity"]
    ...
    table_df["Current Anomaly"] = table_df[
...
<div class="profile-card">
    ...
    <div class="profile-grid">
        ...
        <div class="profile-field">
            <div class="pf-label">24-Hour Risk</div>
            <div class="pf-value">{fmt_pct(row.get("_future_prob_24h"))}</div>
        </div>
        <div class="profile-field">
            <div class="pf-label">3-Day Risk</div>
            <div class="pf-value">{fmt_pct(row.get("_future_prob_3d"))}</div>
        </div>
        <div class="profile-field">
            <div class="pf-label">7-Day Risk</div>
            <div class="pf-value">{fmt_pct(row.get("_future_prob_7d"))}</div>
        </div>
        <div class="profile-field">
            <div class="pf-label">Expected Severity</div>
            <div class="pf-value">{escape(str(row.get("future_unavailability_severity") or "LOW"))}</div>
        </div>
        <div class="profile-field">
        ...
        <div class="profile-field">
            <div class="pf-label">Highest-Risk Window</div>
            <div class="pf-value">{escape(str(row.get("future_risk_window") or "N/A"))}</div>
        </div>
        <div class="profile-field">
            <div class="pf-label">Current Anomaly</div>
            <div class="pf-value">{escape(str(row.get("current_anomaly_status") or "CURRENT_NORMAL"))}</div>
        </div>
        <div class="profile-field">
